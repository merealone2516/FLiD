...
from configs.paths import EFFICIENCY_OUTPUT, YOLO11_WEIGHTS
...
def main():
    ...
    # ── Key ratios ──
    print(f"\n  KEY RATIOS:")
    print(f"    Param ratio:  {trainable_bl / face_trainable:.0f}× fewer trainable params (FLiD single-field detector)")
    print(f"    Param ratio:  {(2 * trainable_bl) / (face_trainable + text_trainable):.1f}× fewer trainable params (FLiD combined heads vs 2× MobileNetV2 baseline)")
    print(f"    FLOP ratio:   {flops_bl / face_total_flops:.1f}× fewer FLOPs (FLiD single field vs baseline pass)")
    print(f"    FLOP ratio:   {cascade_flops / both_total_flops:.1f}× fewer FLOPs (FLiD combined vs 2-pass baseline)")
