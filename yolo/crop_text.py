...
def detect_and_crop_text(model, image_path, conf=0.5):
    ...
    if results:
        for box in results[0].boxes:
            if CLASS_NAMES.get(int(box.cls), '') != 'text':
                continue
            ...
            if crop.size > 0:
                texts.append({'image': crop, 'bbox': [x1, y1, x2, y2],
                              'confidence': float(box.conf)})
    return texts or None


def main():
    parser = argparse.ArgumentParser(description='Crop text regions with YOLO')
    parser.add_argument('--input_dir', default=str(PAIR_DATA / 'Text_attack'))
    parser.add_argument('--output_dir', default=str(PAIR_DATA_YOLO / 'text_attack'))
    ...
    for cat in ['Real', 'Fake']:
        ...
        for img_file in tqdm(imgs, desc=f"Text {cat}"):
            stats['total'] += 1
            texts = detect_and_crop_text(model, img_file, args.conf)
            if not texts:
            ...
            Image.fromarray(best['image']).save(img_out / 'text_crop.png')
    ...
    print(f"\nProcessed {stats['total']} images, detected text in {stats['detected']}")
    if stats['failed']:
